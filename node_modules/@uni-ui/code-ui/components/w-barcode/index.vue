...
<script setup name="WBarcode">
	...
	const GetCodeImg = async ()=> {
		try{
		  	return await GetImg({id: info.id,orient: info.orient,source: platform(),width: opt.orient == 'vertical' ? info.height : info.width,height: info.orient == 'vertical' ? info.width  : info.height,ctx: that});
...
<!-- #ifndef VUE3 -->
<script>
	import { BarCode, GetImg,GetPixelRatio,GetPx } from '@uni-ui/code-plugs';
	...
	export default {
		...
		methods: {
			...
			async GetCodeImg (){
				try{
					return  await GetImg({id: this.id,source: platform(),width: this.info.orient == 'vertical' ? this.info.height : this.info.width,height: this.info.orient == 'vertical' ? this.info.width : this.info.height,ctx: this});
				}catch(e){}
			}
		}
	}
